...
const emptySchedule = () =>
  Object.fromEntries(
    defaultHours.map(hour => [
      hour,
      Object.fromEntries(
        defaultDays.map(day => [day, { subject: "", group: "" }])
      ),
    ])
  );
const CELL_WIDTH = 80;
      ...
      <div className="flex items-center mb-8 gap-3 justify-between my-0 py-[2px]">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={teachers.length <= 1 || editing}
          >
                      ...
                      ? (
                        <div className="flex flex-col gap-1 items-center justify-center">
                          <input
                            ...
                            className="border border-gray-300 text-xs w-full rounded px-1 text-center"
                            style={{ width: "62px" }}
                            onChange={e =>
                              handleCellChange(hour, day, "subject", e.target.value)
                            }
                          />
                          <input
                            type="text"
                            ...
                            style={{ width: "62px" }}
                            onChange={e =>
                              handleCellChange(hour, day, "group", e.target.value)
                            }
                          />
                        </div>
                      )
                        ...
                        (scheduleToShow?.[hour]?.[day]?.subject || scheduleToShow?.[hour]?.[day]?.group)
                          ? (
                            <span>
                              ...
                              {scheduleToShow[hour][day].group && (
                                <span className="block text-xs text-gray-500">{scheduleToShow[hour][day].group}</span>
                              )}
                            </span>
                          ) : (
