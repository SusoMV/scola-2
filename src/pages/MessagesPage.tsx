...
const MessagesPage = () => {
    ...
    const fetchData = async () => {
      ...
      if (user) {
        try {
          ...
          } = await supabase.from('profiles').select('full_name, school_id').eq('id', user.id);
          if (currentUserError) {
          ...
          if (currentUserData && currentUserData.length > 0) {
            ...
            const {
              data: facultyData,
              error: facultyError
            } = await supabase.from('profiles').select('id, full_name, role').eq('school_id', currentUserData[0].school_id).neq('id', user.id);
            if (facultyError) {
              throw facultyError;
            }
  ...
  const handleNewMessage = (data: {
    ...
    if (recipient) {
      const existingConversation = conversations.find(convo => !convo.isGroup && convo.participants.some(p => p.id === recipient.id));
      if (existingConversation) {
        ...
        const updatedConversations = conversations.map(convo => convo.id === existingConversation.id ? {
          ...convo,
  ...
  const handleNewGroup = (data: {
    ...
    const selectedParticipants = data.participants.map((id: string) => facultyMembers.find(member => member.id === id)).filter(Boolean) as Participant[];
  ...
  const handleSendMessage = (messageInput: string) => {
    ...
    const updatedConversations = conversations.map(convo => convo.id === selectedConversation.id ? {
      ...convo,
      messages: [...convo.messages, newMessage],
      lastMessage: {
        content: messageInput,
  ...
  return <DashboardLayout>
      <div className="mb-6 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="border-scola-primary text-scola-primary" onClick={() => setOpenNewGroupDialog(true)}>
            <Users className="mr-2 h-4 w-4" /> Crear grupo
          </Button>
          <Button className="bg-scola-primary hover:bg-scola-primary/90" onClick={() => setOpenNewMessageDialog(true)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <ConversationList conversations={conversations} selectedConversation={selectedConversation} setSelectedConversation={setSelectedConversation} isLoading={isLoading} formatDate={formatDate} />

        <ChatArea selectedConversation={selectedConversation} formatDate={formatDate} onSendMessage={handleSendMessage} />
      </div>

      <NewMessageDialog open={openNewMessageDialog} onOpenChange={setOpenNewMessageDialog} facultyMembers={facultyMembers} onSubmit={handleNewMessage} />
