      ...
      <CardContent className="p-0 flex-1 overflow-hidden">
        <ScrollArea className="h-full">
          {conversations.length > 0 ? <ul className="divide-y divide-gray-200">
              {conversations.map(conversation => <motion.li key={conversation.id} initial={{
            opacity: 0,
            y: 5
          }} animate={{
            opacity: 1,
            ...
            duration: 0.2
                  ...
                  <button className={`w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors ${selectedConversation === conversation.id ? 'bg-gray-50' : ''}`} onClick={() => onSelectConversation(conversation.id)}>
                    <div className="flex items-start">
                      <div className={`rounded-full w-10 h-10 flex items-center justify-center mr-3 flex-shrink-0 ${conversation.isGroup ? 'bg-scola-pastel text-scola-primary' : 'bg-scola-pastel text-scola-primary'}`}>
                  ...
                  {onDeleteConversation && <button className="absolute top-3 right-3 text-red-500 opacity-0 group-hover:opacity-100 hover:opacity-100 focus:opacity-100 transition-opacity" onClick={e => {
              e.stopPropagation();
